...
const HomeVehicle = ({ vehicle }) => {
  ...
  return (
    <div className="mb-5 col-lg-4 col-sm-6 ">
      <div className="card shadow-lg w-100 h-100 text-center rounded bgcolor">
        ...
        <div className="card-footer">
          <div className="d-flex align-items-center justify-content-between ">
            <Link to={`/vehicle/${_id}`}>
              <button className="btn px- fw-bold btn-danger">View Detail</button>
            </Link>
            <Link to={`/order/${_id}`}>
              <button className="btn fw-bold btn-primary">Buy Now</button>
            </Link>
          </div>
        </div>
      </div>
